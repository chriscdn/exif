...
import { toNumberOrThrow as toNumberOrThrow2 } from "@chriscdn/to-number";
var isValidExifDateFormat = (dateStr) => /^\d{4}:\d{2}:\d{2} \d{2}:\d{2}:\d{2}$/.test(dateStr);
var parseIsoDateTimeAndOffset = (isoString) => {
...
var exifDateToISODate = (exifDateString) => {
  if (isValidExifDateFormat(exifDateString)) {
    const [datePart, timePart] = exifDateString.split(" ");
    const dateSplit = datePart?.split(":") ?? [];
    const timeSplit = timePart?.split(":") ?? [];
    const year = toNumberOrThrow2(dateSplit[0]);
    const month = toNumberOrThrow2(dateSplit[1]);
    const day = toNumberOrThrow2(dateSplit[2]);
    const hour = toNumberOrThrow2(timeSplit[0]);
    const minute = toNumberOrThrow2(timeSplit[1]);
    const seconds = toNumberOrThrow2(timeSplit[2]);
    return {
